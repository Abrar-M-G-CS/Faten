...
/* Loads trained model */
async function init() {  
  model = await tf.loadModel('http://localhost:3232/tfjs-models/model-character/model.json');
}
...
function give_verdict() {
  ...
  if (digit == bestPredIndex && Math.floor(bestPred * 10) >= 5) {
      const arabicPrediction = arabicCharacters[bestPredIndex];
      const arabicSuccessMessage = `أحسنت! لقد حصلت على ${arabicPrediction} من ١٠`;
      displayBox.innerText = arabicSuccessMessage;
  } else {
      displayBox.innerText = `أوه! انت تفعلها بشكل خاطئ. حاول مرة اخرى`;
  }
}
